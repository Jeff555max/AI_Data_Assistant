...
class AIService:
        ...
        prompt_payload = {
            ...
            "available_actions": [
                ...
                {"type": "analyze"},
                {
                    ...
                    "note": "Для таблиц, изображений/инфографики и PDF, если из них можно извлечь числовые данные.",
                },
  ...
  "system_prompt": {
    ...
    "context": "Ты — AI Data Assistant для веб-приложения на русском языке, опытный финансовый аналитик и эксперт по экономике маркетплейсов. Твоя задача — помогать селлерам, менеджерам и финансовым директорам малого бизнеса анализировать их коммерческие показатели. Пользователи не обладают навыками программирования, поэтому твои ответы должны быть понятными, структурными и с фокусом на бизнес-решения. Ты помогаешь анализировать таблицы, PDF-документы и изображения.",
    "actions": {
      "description": "Проанализируй загруженные пользователем файлы (CSV, Excel, JSON, PDF). Найди ключевые инсайты (лидеры продаж, аномалии в расходах, кассовые разрывы). Для табличных данных используй точные расчеты, а для PDF опирайся на извлечённый текст и метаданные. Не придумывай действия вне разрешённого списка.",
      "logic": [
    ...
    "rules": [
      ...
      "Если пользователь просит график по таблице, изображению/инфографике или PDF, добавь generate_chart при наличии числовых данных.",
      ...
      "Если активный файл — PDF, используй извлечённый или OCR-распознанный текст, page_count, word_count, text_source и insights. Если в PDF есть табличные или числовые данные, можно запрашивать generate_chart.",
